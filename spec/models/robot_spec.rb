...
describe Robot do
  let(:not_placed_msg) { 'The robot has not been sucessfully placed yet' }
  ...
  describe '#place' do
    ...
    it 'does not set a position for an invalid place' do
      expect(Robot.new.place(7, 3, 'NORTH').report).to eq not_placed_msg
    end
  end
  ...
  describe '#move' do
    ...
    it 'ignores move when a successful place has not occured' do
      expect(Robot.new.move.report).to eq not_placed_msg
    end
  end
  ...
  describe '#left & #right' do
    ...
    it 'turns left 360 degrees / 4 times' do
      robot.left
      expect(robot.report).to eq '3, 3, WEST'
      robot.left
      expect(robot.report).to eq '3, 3, SOUTH'
      robot.left
      expect(robot.report).to eq '3, 3, EAST'
      robot.left
  ...
  describe '#input' do
    it 'commands_1' do
      expect(Robot.new.input(File.open('./spec/fixtures/commands_1.yml'))
        .report).to eq '0, 1, NORTH'
    end

    it 'commands_2' do
      expect(Robot.new.input(File.open('./spec/fixtures/commands_2.yml'))
        .report).to eq '0, 0, WEST'
    end

    it 'commands_3' do
      expect(Robot.new.input(File.open('./spec/fixtures/commands_3.yml'))
        .report).to eq '3, 3, NORTH'
    end
  end
  ...
  it '#position?' do
    expect(Robot.new.place(3, 3, 'NORTH').send(:position?)).to be true
    expect(Robot.new.place(3, 6, 'NORTH').send(:position?)).to be false
  end
end
